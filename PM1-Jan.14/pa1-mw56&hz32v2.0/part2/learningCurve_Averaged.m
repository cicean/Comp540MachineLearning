...
for num_ex = 1:m
    ...
    for i = 1:iters
        train_inds = randperm(m,num_ex); 
        X_subset = X(train_inds,:);
        y_subset = y(train_inds);

        val_inds = randperm(mval,min(mval, num_ex));
        Xval_subset = Xval(val_inds,:);
        yval_subset = yval(val_inds);

        theta = trainLinearReg(X_subset, y_subset, lambda);

        error_train_iter(i) = linearRegCostFunction(X_subset,y_subset,theta,0);
        error_val_iter(i) = linearRegCostFunction(Xval_subset,yval_subset,theta, 0);
